...
function Createv() {
  ...
  const [previewSource, setPreviewSource] = useState("https://cdn.pixabay.com/photo/2017/01/18/17/39/cloud-computing-1990405_960_720.png");
...
const getImage =  ()=> {
  ...
  const [event, setEvent] = useState({
    title: "", description:"", image:"", isoffline:"online",venue:"", link:"", date:"",time:"",eventspeaker:"",contact:"",ispaid:"free",cost:"",isfeatured:"no",tags:""
  });
  let name, value;
  ...
  const PostData = async(e) => {
    ...
    const  { title, description, image, cost, venue, eventspeaker, contact, tags, ispaid, isoffline,link, isfeatured, date, time}=event;
    console.log(event);
    const res= await fetch("/createv",{
        ...
        body: JSON.stringify({
            title,description,image,cost,venue,eventspeaker,contact,tags,ispaid,isoffline,link,isfeatured,date,time
          })
      });
    ...
    <Box className={classes.main}>
      ...
      <Box className={classes.uploadImage}>
        ...
        <label htmlFor="contained-button-file">
          <Input accept="image/*" id="contained-button-file" multiple type="file" onChange={handlePhotoInputs}/>
          <Button   variant="contained" component="span" className={classes.btn}>Upload</Button>
        </label>
        <Button  type="submit"  className={classes.submitImg} color="secondary" onClick={handleSubmitFile}>Submit</Button>
      </Box>
    </Box>
